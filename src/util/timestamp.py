...
class Timestamp:
  ...
  def _add_number(self, num: int) -> 'Timestamp':
    logging.warning((
      'Add number to timestamp might make some mistake,',
      'as the unit of number is not clear. Please use Timedelta to avoid any confuse.'
    ))
  ...
  def __add__(self, other: Union[int, 'Timedelta']) -> 'Timestamp':
    if isinstance(other, int):
      ...
      raise WServiceError(
        ...
        'unsupported operand type(s) for +: \'Timestamp\' and \'' + str(type(other)) + '\''
      )
