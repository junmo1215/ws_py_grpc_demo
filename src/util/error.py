...
from src.util.pb_util import PbHelper
import logging
import grpc
...
class ErrorCode(Enum):
  ...
  UNINITED = 101 # 没有初始化
  EXTERNAL_SERVICE_ERROR = 102 # 外部依赖的服务异常

class WServiceError(Exception):
  """ 服务端内部异常

  try:
    raise WServiceError(ErrorCode.xxx, "")
  except WServiceError as ex:
    code, detail = ex.args
  """

def catch_inner_error_wrapper(return_type=None):
  def decorator(func):
    def grpc_wrapper(self, request, context):
      try:
        logging.debug("Call %s() %s", func.__name__, PbHelper.pb2str(request))
        resp =  func(self, request, context)
        logging.debug("%s Response %s", func.__name__, PbHelper.pb2str(resp))
        return resp
      except grpc.RpcError as ex:
        code, detail = ex.args # pylint: disable=unbalanced-tuple-unpacking
        context.set_code(code)
        context.set_details(detail)
        logging.warning(
          "Call %s() error. request: %s, msg: %s",
          func.__name__, PbHelper.pb2str(request), detail
        )
